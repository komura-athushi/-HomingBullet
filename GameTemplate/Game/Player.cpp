...
void Player::TakeAim()
{
	...
	//ターゲッティングしない。
	//近すぎてもターゲッティングしない。
	if (diff.Length() >= 1500.0f || diff.Length() <= 20.0f)
	{
		m_isTakeAim = false;
		return;
	}
...
void Player::Shoot()
{
	...
	auto bullet = NewGO<Bullet>(0);
	bullet->SetPosition(m_position);
	//ランダムな方向に弾を射出。
	Quaternion rotation;
	rotation.SetRotationDeg(m_forward, rand()%360);
	rotation.Apply(m_right);
	bullet->SetVelocity(m_right * 10);
	bullet->SetTarget(m_targetPosition);
